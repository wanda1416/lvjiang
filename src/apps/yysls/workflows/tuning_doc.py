...
# 品阶 key → 中文名
_QUALITY_NAMES = {"gold": "金色", "purple": "紫色", "blue": "蓝色"}
...
class TuningDocWriter:
    ...
    def start_run(self, username: str, schools_desc: list[str],
                  slots: list[str], keep_pvp: bool):
        """文档头：开始时间、操作用户、启用流派及武器规则、部位"""
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self._write(f"# 调律说明 — {now}")
        self._write()
        self._write(f"- 操作用户：{username}")
        schools = "、".join(schools_desc) if schools_desc else "全部流派（默认配置）"
        self._write(f"- 启用流派：{schools}")
    ...
    def worthiness_matched(self, results: dict[str, dict]):
        """符合的规则：只写命中 顶级/优秀 的流派（不写不符合的）

        Args:
            results: judge_equipment_potential 的结构化结果
        """
        self._write()
        self._write("符合以下规则，开始调律：")
    ...
    def finish_equipment(self, rounds: int, affix_count: int,
        ...
        for r in (judgement or {}).values():
            if r.get("not_applicable"):
                continue
            tag = "跳过" if r.get("skipped") else r.get("rating", "")
            detail = "；".join(r.get("reasons") or [])
            lines.append(f"{r['name']}：{tag}（{detail}）")
        self._write(f"最终评级：{'；'.join(lines) if lines else '无有效结论'}")
    ...
    def end_run(self, interrupted: bool, tuned_count: int, total_rounds: int):
        ...
        state = "用户中断（F10）" if interrupted else "正常完成"
        self._write()
        self._write("## 运行结束")
        self._write()

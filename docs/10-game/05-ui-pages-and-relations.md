...
| `general_combat` | 战斗操作 | 切换武器、武学技、特殊技、奇术、卸势、蓄力、轻击、闪避、跳跃、弓箭 | 独立的单视图战斗场景，供桌面按键与 Android 触控统一寻址 |
| `general_move` | 移动操作 | 角色移动与镜头转向锚点 | 独立的单视图移动场景，桌面对应 WASD/鼠标，Android 对应左右屏拖动 |
| `general_control` | 全局共享控件/叠层 | 确认、取消、关闭物品、空白区、菜单滚动 | 不是单一页面；被登录、购买、回收、重置、任务等流程复用 |
...
| `appearance_main` | 角色外观 | 衣柜（基底）、情境 tab；各自独立的方案编辑页 | 号令换装与情境任务共用 |
| `baiye_main` | 百业 | 返回驻地、百业菜单、活动视图 | 周常货运流程仍标记 WIP |

外观主页面的 `base` 即衣柜入口，`qingjing` 是同层的第二个 tab；衣柜的
“穿搭方案”进入非同层 `chuanda`，情境的“编辑情境”进入非同层
`qingjing_editing`。四个视图共用同位置的 `back` 返回区域，主页面 tab 不归属编辑页。
情境编辑正常返回情境 tab；既有号令直达流程则按直接返回号令情境层处理。
为兼容两种入口，不声明单一固定的返回 `to`；工作流退出编辑后识别“编辑情境”，
仅在仍处于外观情境 tab 时退出外观，再继续返回号令。

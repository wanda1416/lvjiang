...
## by image — 模板定位（图色三件套的"图"）

`find` 除 OCR 文字搜索外，还支持按**模板图片**定位：

```
find as $icon by image "extract_icon"
find [map].[canvas] as $icon by image "extract_icon" where confidence >= 0.85
if $icon
    click $icon
end
```
...
| 项 | 说明 |
|----|------|
| target | 模板名（不含扩展名），对应 `config/system/templates/<name>.png`；用户层可在 `config/local/templates/` 覆盖 |
| sidecar | 可选 `<name>.json`：`{"recordW": 2400, "recordH": 1080}` 记录录制时的画布宽高，运行时按 `当前画布宽 / recordW` 缩放模板再匹配（±10% 各试一次，并用 1.0 兜底） |
| where | `confidence >= x` 是匹配分门槛（`TM_CCOEFF_NORMED`，0–1），缺省 0.8 |
| 返回 | `FoundRegion`（命中模板的归一化矩形，`text` 字段 = 模板名）或 `""` |
| 模板不存在 | 直接报错，不静默返回 `""`——这是脚本作者的错，不是画面没出现 |

`by image` **只属于 find**；`scan` / `recognize` 写 `by image` 在解析阶段报错。

与 `recognize` 的区别：`recognize` 是"这块区域最像图库里哪一条"（分类，ORB 特征），
`find … by image` 是"这张小图出现在画面哪里"（定位，灰度相关）。要找位置不固定的
图标（地图标记、弹窗按钮），用后者；同类图标一次找全，用内置函数 `find_icons`（见

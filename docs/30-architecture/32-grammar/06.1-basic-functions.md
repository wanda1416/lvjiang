...
### extract_int / extract_num — 从文本中提取数字

OCR 结果常混入单位、图标或噪点字符。这两个函数用边界正则提取第一项**非负数**，
自动排除负号、千分位逗号等干扰；找不到时返回 `-1`，因此 `0` 与“提取失败”可以区分：

```text
eval $level = extract_int("等级 110阶")      # 110
eval $count = extract_num("0/691")           # 0
eval $score = extract_num("评分 1234.5")     # 1234.5（float）
```

| 函数 | 返回 | 说明 |
|------|------|------|
| `extract_int($s)` | `int` | 只提取整数，小数会被排除 |
| `extract_num($s)` | `int \| float` | 允许小数点，含 `.` 返回 float，否则 int |

### extract_progress — 提取 `当前/总量`

解析 `0/691`、`12 ／ 30`（兼容全角斜杠与斜杠两侧空白）这类进度文本：

```text
eval $p = extract_progress("0/691")
# $p = {"valid": true, "current": 0, "total": 691}
if $p.valid
    log concat("进度: ", $p.current, "/", $p.total)
end
```

候选不唯一或不合法（`current > total`、`total <= 0`）时返回
`{"valid": false, "current": -1, "total": -1}`，调用方应先用 `$p.valid` 判断。

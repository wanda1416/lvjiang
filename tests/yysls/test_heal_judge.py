"""治疗规则（治疗-纯奶 / 治疗-火拳奶）判定器测试（通用规则引擎）

覆盖 04-治疗纯奶调律说明.md / 05-治疗火拳奶调律说明.md
第九节全部判定例子，以及两规则独立判定、品阶/首词条筛选、
调律潜力判定等场景。
"""
...
class TestIndependentRules:
    def test_fan_pure_top_fire_junk(self, pure, fire):
        # 扇武学增效 是纯奶主武器必需增伤，对火拳奶是废神力词条
        e = make_equip("扇", ["最大外功攻击", "扇武学增效", "劲", "最小外功攻击", "劲"])
        assert pure.judge(e).rating == Rating.TOP
        assert fire.judge(e).rating == Rating.JUNK

    def test_fan_fire_top_pure_junk(self, pure, fire):
        # 纯奶主武器缺 扇武学增效 → 垃圾；火拳奶不要求 → 顶级
        e = make_equip("扇", ["最大外功攻击", "最大外功攻击", "劲", "敏", "敏"])
        assert fire.judge(e).rating == Rating.TOP
        assert pure.judge(e).rating == Rating.JUNK

    def test_ring_pure_top(self, pure):
        e = make_equip("环", ["最大外功攻击", "全武学增效", "劲", "劲", "会心率"])
        assert pure.judge(e).rating == Rating.TOP

    def test_pendant_fire_top_pure_junk(self, pure, fire):
...
class TestScreening:
    def test_jin_first_fan_skipped(self, pure):
        e = make_equip("扇", ["劲", "最大外功攻击", "劲", "敏", "势"])
        r = pure.judge(e)
        assert r.skipped and not r.not_applicable

    def test_purple_weapon_skipped(self, pure):
        e = make_equip("扇", ["最大外功攻击", "扇武学增效", "劲", "最小外功攻击", "劲"],
                       quality="purple")
        assert pure.judge(e).skipped

    def test_jian_not_applicable(self, pure):
    ...
    def test_leg_player_effect_not_pvp_flag(self, pure):
        # 对玩家增效是纯奶胫甲的必需词条而非 PVP 保留（heal_pure 无 keep_pvp）
...
class TestTuningWorthiness:
    def test_fan_with_damage_top(self, pure):
    ...
    def test_junk_affix_transmutable(self, pure):
        # 会意率 在纯奶库外，可转律洗掉 → 仍可达顶级
        e = make_equip("伞", ["最大外功攻击", "劲", "会意率"])
        r = pure.check_tuning_worthiness(e)
        assert r.rating == Rating.TOP
        assert any("模拟转律" in s for s in r.reasons)

    def test_double_junk_ceiling(self, pure):
        e = make_equip("伞", ["最大外功攻击", "劲", "会意率", "最大鸣金攻击"])
        assert pure.check_tuning_worthiness(e).rating == Rating.JUNK

    def test_dawuxiang_transmutable_top(self, pure):
    ...
    def test_worthiness_via_heal_rules(self):
        e = make_equip("扇", ["最大外功攻击", "扇武学增效"])
        worth, logs = judge_tuning_worthiness(
            e, schools=["heal_pure", "heal_fire"])
        assert worth

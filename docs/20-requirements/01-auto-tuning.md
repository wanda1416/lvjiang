...
扫描处理、材料处理、调律处理三个规则表的「添加规则」行最右侧均提供
「校验规则」按钮。结果区域默认隐藏；点击后只列出部位 × 金/紫品阶
完全没有启用候选规则的组合，按钮变为「清理结果」，再次点击隐藏结果。
切换规则组或编辑规则时清理旧结果。判定语义、评级、首词条数值和调律准入
门槛取决于实际装备与运行参数，不得被静态检查推断为必然命中。

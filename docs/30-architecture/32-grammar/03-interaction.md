...
| **place** | 鼠标 | 直接放置光标 | `move` 的辅助原语 |
| **move** | 鼠标 | 移动鼠标到目标位置 | 悬停触发 tooltip、安全区检查 |
| **scroll** | 鼠标滚轮 | 在目标位置滚动滚轮 | 列表翻页、配合 move 精确定位滚动 |
| **drag** | 鼠标/触摸 | 拖拽（翻页/滑动） | 滚动列表、翻页 |
| **replay** | 输入轨迹 | 回放 `.lvtrace` 高精度轨迹 | 录制的高保真操作序列 |
| **wait** | 时间 | 暂停执行 | 等动画、等加载 |
| **press** | 键盘/鼠标键 | 模拟可按压控件 | 快捷键、鼠标键、长按、组合键 |
| **paste** | 键盘 | 剪贴板 + Ctrl+V 文本输入 | 兑换码、角色名 |
| **align** | 图像 | 面板自对齐 | 计算网格坐标（通常自动触发） |
| **screenshot** | 截屏 | 保存当前画面 | 调试记录 |
| **app** | 进程 | 应用停止/启动 | 冷启动、崩溃重启 |

按输入通道分组：
- **基础指令**（collect / eval / call / log）→ 数据操作与控制流
- **鼠标操作**（click / place / move / scroll / drag）→ 通过坐标注入鼠标/触摸事件；`place` 是 `move` 的放置原语
- **时间与辅助**（wait / wait stable / align / screenshot）→ 节奏控制与校准
- **按压与文本输入**（press / paste）→ `press` 统一处理键盘键和桌面鼠标按钮；`paste` 走剪贴板 + `Ctrl+V`
- **进程与轨迹**（app / replay）→ 应用生命周期控制、`.lvtrace` 高精度轨迹回放
...
press 是唯一的按压指令，通过 **KeyStateRegistry** 统一管理键盘键和鼠标按钮状态：

...
- 主线功能全部就绪：自动调律端到端流水线（背包遍历 → 潜力判定 →
  实际调律 → 终局判定 → 调律说明文档）可用。
- pytest **1051 例全绿**（2026-08-01 本地全量验证）。
- 全部改动已提交（最新 `2856ce5`）；是否推送以 /submit 指令为准。
- 严禁自动提交；提交时中文信息须写入 UTF-8 文件后 `git commit -F`。

## 二、最近完成（2026-07-31 ~ 08-01）

- **布局存储目录化**（08-01 `2856ce5`）：单文件 `默认布局.json`（2001 行）

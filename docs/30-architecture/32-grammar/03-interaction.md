...
| **mouse** | 鼠标 | 鼠标键原始 down/up | 录制器还原多输入交叠时间线 |
...
- **鼠标操作**（click / mouse / place / move / scroll / drag）→ 通过坐标注入鼠标/触摸事件；`mouse` 保留桌面鼠标键的原始 down/up，`place` 是 `move` 的放置原语
- **时间与辅助**（wait / wait stable / align / screenshot）→ 节奏控制与校准
- **键盘输入**（press / paste）→ 通过 VK 码注入键盘事件；`paste` 走剪贴板 + `Ctrl+V`

...
地图定义走 system / local / remote 三层：system 只读、"复制到本地"后修改；
...
north_up: true
base_image: base.png
base_image_sha256: 0123...cdef          # 导入时自动生成
ui:
  scene: map_duchenxu
  minimap: minimap
...
1. 以布局标定的 `minimap_center` 为中心按 HSV 取色；未标定时才退化为小地图
   区域中心并显著提示；取离中心最近且面积足够的连通块；
...
缩放下最大误差 2.1°；这只是算法单元测试，不等于真机验收。真实截图样本、黄色
干扰物、遮挡及负样本通过离线门禁前，朝向解析只能用于诊断，不能驱动角色移动。
视角光锥不需要识别：移动时箭头 = 跑动方向，镜头朝向可由"箭头方向 − 推杆角"
反推（见需求文档）。

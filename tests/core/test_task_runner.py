...
def test_build_source_missing_wf_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "lvjiang.constants.SYSTEM_WORKFLOWS_DIR", tmp_path, raising=False
    )
...
def test_device_smoke_wf_runs_through_task_runner(monkeypatch):
    ...
    assert _wait_until(lambda: _status()["state"] != task_runner.STATE_RUNNING, 30.0)

    status = _status()
    assert status["state"] == task_runner.STATE_DONE, status["message"]
    # collect 的结果要真的回到状态里，否则悬浮面板只能看到一个空 done
    assert status["result"].get("count") in (5, 5.0)
    assert engine.variables["msg"] == "hello from device"

...
安卓 ADB 运行模式还提供应用生命周期函数：

```text
app stop "game" timeout 15
app start "game" timeout 30
eval android_wait_stable_frame("game", 90, 1)
if android_app_running("game")
    log "应用正在运行"
end
```

其中 `game` 是「配置管理 → 安卓设置」注册的应用名。`stop` 会等待进程消失，
`start` 会等待进程出现；`android_wait_stable_frame` 会等待 scrcpy/截图后端恢复、
画面达到应用配置的期望方向，并连续稳定指定秒数。非 ADB PC 运行模式调用会直接报错。

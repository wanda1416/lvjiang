...
ACTIONS: tuple[Action, ...] = (
    # ── 交互 ──
    Action("app_stop", tr("停止应用"), CAT_INTERACT,
           "app stop {name}{timeout}",
           (Slot("name", tr("应用名"), "text", default="game"),
            Slot("timeout", tr("超时(秒)"), "raw", optional=True,
                 wrap=" timeout {v}")),
           doc=tr("按应用注册类型停止 Android 或 PC 应用，并确认进程消失"),
           keywords=("app", "android", "adb", "pc", "stop")),
    Action("app_start", tr("启动应用"), CAT_INTERACT,
           "app start {name}{timeout}",
           (Slot("name", tr("应用名"), "text", default="game"),
            Slot("timeout", tr("超时(秒)"), "raw", optional=True,
                 wrap=" timeout {v}")),
           doc=tr("按应用注册类型启动 Android 或 PC 应用，并确认进程出现"),
           keywords=("app", "android", "adb", "pc", "start")),
    Action("click_region", tr("激活区域"), CAT_INTERACT,
           "click {scene}.{region}{hold}{wait}",
           (Slot("scene", tr("场景"), "scene"),
            Slot("region", tr("区域/点"), "region"),
            Slot("hold", tr("按住(秒/区间)"), "raw", optional=True,
                 wrap=" hold {v}"), _wait_slot()),
           doc=tr("激活布局实体；默认点击中心，布局可改为按键"), keywords=("click",)),
    Action("click_coord", tr("点击坐标"), CAT_INTERACT,
           "click {coord}{hold}{wait}",
           (Slot("coord", tr("坐标"), "coord", help=tr("画布上点一下取值")),
            Slot("hold", tr("按住(秒/区间)"), "raw", optional=True,
                 wrap=" hold {v}"), _wait_slot()),
           doc=tr("点击画布归一化坐标"), keywords=("click",)),
    Action("click_var", tr("点击变量"), CAT_INTERACT,
           "click {var}{hold}{wait}",
           (Slot("var", tr("变量"), "var", help=tr("find 结果、矩形或坐标变量")),
            Slot("hold", tr("按住(秒/区间)"), "raw", optional=True,
                 wrap=" hold {v}"), _wait_slot()),
           keywords=("click",)),
    Action("drag_coords", tr("拖拽（两点）"), CAT_INTERACT,
           "drag {from_} to {to}{scale}{dur}{hold}{exact}",
           (Slot("from_", tr("起点"), "coord"), Slot("to", tr("终点"), "coord"),
            Slot("scale", tr("向量倍数"), "raw", optional=True, wrap=" scale {v}",
                 help=tr("起点→终点向量放大/缩小，如 2 或 0.5")),
            Slot("dur", tr("滑动时长(秒/区间)"), "raw", optional=True, wrap=" duration {v}"),
            Slot("hold", tr("到达后按住(秒)"), "raw", optional=True, wrap=" hold {v}"),
            Slot("exact", tr("不抖动"), "choice", default="", choices=("", "exact"),
                 optional=True, wrap=" {v}")),
           keywords=("drag", "swipe")),
    ...
    Action("press", tr("按压按键/鼠标键"), CAT_INTERACT,
           'press "{key}"{mode}',
           (Slot("key", tr("键名"), "raw", default="ESC", help=tr("ESC / 字母键 / MOUSE_LEFT；设备端 ESC=返回")),
            Slot("mode", tr("模式"), "choice", default="",
                 choices=("", "hold 0.5", "down", "up"), optional=True,
                 wrap=" {v}")),
           keywords=("press", "key", "mouse", "button")),
    Action("paste", tr("粘贴文本"), CAT_INTERACT,
           "paste {text}{wait}",
           (Slot("text", tr("文本/变量"), "raw", default='"ABC123"',
                 help=tr('字符串写双引号，或使用 $变量')),
            _wait_slot()),
           doc=tr("PC 端通过剪贴板和 Ctrl+V 输入文本，支持后台模式"),
           keywords=("paste", "text", "clipboard")),
    ...
    Action("scan_image", tr("模板匹配区域"), CAT_PERCEIVE,
           "scan {scene}.{region} as {var} by image{conf}",
           (Slot("scene", tr("场景"), "scene"), Slot("region", tr("区域"), "region"),
            Slot("var", tr("存入变量"), "var", default="hit"),
            Slot("conf", tr("最低分(0-1)"), "raw", optional=True,
                 wrap=" where confidence >= {v}")),
           doc=tr("按布局中 Region 绑定的模板匹配，返回首个命中的区域 key"),
           keywords=("scan", "image", "template")),
    ...
    Action("find_bound_image", tr("按区域绑定找图"), CAT_PERCEIVE,
           "find{area} as {var} by image {scene}.{region}{conf}",
           (Slot("area", tr("搜索范围"), "raw", optional=True, wrap=" {v}",
                 help=tr("形如 [scene].[region]；留空则搜索整个画布")),
            Slot("var", tr("存入变量"), "var", default="icon"),
            Slot("scene", tr("模板场景"), "scene"),
            Slot("region", tr("模板区域"), "region"),
            Slot("conf", tr("最低分(0-1)"), "raw", optional=True,
                 wrap=" where confidence >= {v}")),
           doc=tr("读取当前布局的 Region 模板绑定，并在指定范围或全画布定位"),
           keywords=("find", "image", "template", "region")),
    ...
    Action("assign", tr("赋值"), CAT_DATA,
           ...
           keywords=("eval", "set")),
    Action("global", tr("声明全局变量"), CAT_DATA,
           "global {var}",
           (Slot("var", tr("变量"), "var", default="shared"),),
           doc=tr("声明后可在主流程与所有子过程中共享读写"),

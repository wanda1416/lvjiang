...
| 方案 | 实现 | 特点 |
|---|---|---|
| **Excel 管线（本文档）** | 四层编译管线 | 纯 Python，透明可审计，依赖策划 Excel |
| **leoq7 WASM** | Rust → WASM 引擎 | 黑盒计算，结果精确但不可审计 |

当前以 Excel 管线为主方案。WASM 逆向分析记录在 `docs/20-requirements/05-graduation-rate-calc.md`，仅作为技术参考。

...
class TestDeletable:
    ...
    def test_factory_entry_blocked_in_user_mode(self, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda: True)
        ok, hint = deletable("出厂甲", self.FACTORY)
        assert not ok
        assert hint
    ...
    def test_dev_mode_allows_everything(self, monkeypatch):
        monkeypatch.setattr(
            "lvjiang.apps.yysls.ui.game_settings.factory_guard.is_user_mode",
            lambda: False)
        assert deletable("出厂甲", self.FACTORY)[0]
    ...
    def test_custom_hint_passed_through(self, monkeypatch):
        monkeypatch.setattr(
            "lvjiang.apps.yysls.ui.game_settings.factory_guard.is_user_mode",
            lambda: True)
        assert deletable("出厂甲", self.FACTORY, hint=READONLY_HINT)[1] == READONLY_HINT
...
class TestTableRowPanels:
    ...
    被直接写进 system，立刻变成"出厂内容"，现实中不会发生。
    """

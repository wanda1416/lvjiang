# 装备领域模型定义

本文档定义装备数据的严格结构化格式，作为 OCR 识别、数据清洗、工作流处理、毕业率计算等模块的共同契约。

> 游戏规则（调律/转律/承音机制等）参见 [装备系统介绍](../../10-game/01-equipment-system.md)。

---

## 一、装备结构定义

一件装备由以下字段组成：

```json
{
  "slot": "main_weapon",
  "type": "剑",
  "level": 110,
  "original_level": 110,
  "cooldown_kind": "reset",
  "cooldown_state": "completed",
  "cooldown_expires_at": "2026-09-07T20:00:00.000+00:00",
  "quality": "gold",
  "is_chengyin": true,
  "lock_status": "locked",
  "base_attr": { "name": "外功攻击", "value": [100, 232] },
  "base_attr_2": null,
  "affix_1": { "name": "最大外功攻击", "value": 114.1, "is_transferred": false },
  "affix_2": { "name": "会意率", "value": 6.6, "unit": "%", "is_transferred": false },
  "affix_3": { "name": "势", "value": 72.2, "is_transferred": true },
  "affix_4": { "name": "最大外功攻击", "value": 114.1, "is_transferred": false },
  "affix_5": { "name": "剑武学增伤", "value": 9.2, "unit": "%", "is_transferred": false }
}
```

---

## 二、字段详细说明

### 2.1 部位 `slot`

共 10 个部位，核心调律考虑前 8 个：

| 键名 | 中文名 | 类别 | 是否考虑 |
|------|--------|------|----------|
| `main_weapon` | 主武器 | 武器 | ✓ |
| `sub_weapon` | 副武器 | 武器 | ✓ |
| `ring` | 环 | 首饰 | ✓ |
| `pendant` | 佩 | 首饰 | ✓ |
| `head` | 冠胄 | 防具 | ✓ |
| `chest` | 胸甲 | 防具 | ✓ |
| `leg` | 胫甲 | 防具 | ✓ |
| `wrist` | 腕甲 | 防具 | ✓ |
| `bow` | 弓 | 特殊 | ✗ |
| `shoot_jade` | 射玦 | 特殊 | ✗ |

### 2.2 类型 `type`

武器类型的枚举值（主武器和副武器共享同一类型）：

| 类型 | 对应流派 |
|------|----------|
| 剑 | 鸣金·虹、鸣金·影 |
| 枪 | 鸣金·虹、鸣金·影、裂石·威 |
| 扇 | 牵丝·玉、牵丝·霖 |
| 陌刀 | 裂石·威、裂石·钧 |
| 双刀 | 破竹·风 |
| 伞 | 牵丝·玉、牵丝·霖、破竹·尘 |
| 绳镖 | 破竹·风、破竹·尘、破竹·鸢 |
| 横刀 | 裂石·钧 |
| 手甲 | 破竹·鸢 |
| 舞绫鼓 | 牵丝·翊 |
| 扇 | 牵丝·翊 |

### 2.3 装备等阶 `level` + `original_level` + `is_chengyin`

| 字段 | 类型 | 说明 |
|------|------|------|
| `level` | int | 装备等阶数字（如 96、100、105、110） |
| `original_level` | int | 原始等级；仅按名称开头的等阶名称识别，无法匹配为 0 |
| `cooldown_kind` | `reset` \| `transmute` \| `unknown` \| `""` | 重置调律、词条转律或无法确定类型；无冷却标记为空 |
| `cooldown_state` | `cooling` \| `completed` \| `""` | 冷却中或冷却完成；无冷却标记为空 |
| `cooldown_expires_at` | string | 冷却到期 UTC ISO 时间；无冷却或剩余时长无法识别时为空 |
| `is_chengyin` | bool | 是否为承音装备 |
| `lock_status` | `"locked"` \| `"unlock"` \| 缺失 | 扫描时识别的锁定状态；旧数据或无法确定时缺失 |

`level` 是当前显示等级，可能受承音影响；`original_level` 不得用当前等级或基础
属性反推。系统配置中，输出装备的“流星”和防具的“吴钩”均识别为原始 110 级。

### 2.4 装备品阶 `quality`

装备的品质稀有度，决定装备的基础色调和上限潜力：

| 枚举值 | 中文名 | 颜色 |
|--------|--------|------|
| `gold` | 金色 | 金/橙色边框 |
| `purple` | 紫色 | 紫色边框 |
| `blue` | 蓝色 | 蓝色边框 |
| `green` | 绿色 | 绿色边框 |

品阶从高到低为：金色 > 紫色 > 蓝色 > 绿色。

### 2.5 基础属性1 `base_attr`

| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | string | 属性名称 |
| `value` | int \| [int, int] | 武器为二元组 `[min, max]`，防具为单一整数 |

**武器基础属性**：
- 名称固定为 `外功攻击`
- 值为取值范围二元组，如 `[100, 232]`

**防具基础属性**：
- 名称为 `气血最大值` 或 `外功防御`
- 值为单一整数

### 2.6 基础属性2 `base_attr_2`（仅防具）

| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | string | 属性名称（固定为 `外功防御`） |
| `value` | int | 属性值 |

**武器和首饰该字段为 `null`**。

### 2.7 词条 `affix_1` ~ `affix_5`

每件装备固定 5 条词条，每条结构：

| 字段 | 类型 | 说明 |
|------|------|------|
| `name` | string | 词条名称（见第三章枚举） |
| `value` | float | 词条数值 |
| `unit` | string \| null | 单位，百分比类为 `"%"`，否则为 `null` |
| `is_transferred` | bool | 是否为转律产出 |
| `cap_pct` | float \| null | 数值占该等级上限的百分比；无上限数据时为 `null` |
| `target_transmute_name` | str（可选） | 模拟转律目标词条名；只是计划，不改变 `name`/`value`，不参与指纹。一件装备至多一条词条带此字段，须与 `target_transmute_value` 同时有效，见 [模拟转律需求](../../20-requirements/09-transmute-simulation.md) |
| `target_transmute_value` | float（可选） | 目标词条按实际装备等级／承音状态的上限值（未承音普通上限、承音承音上限）；计算时按投影副本重新取值 |

---

## 二之二、状态异常标记 `_extra.illegal_equip`

游戏产不出来的装备组合一律标为**状态异常**。判定由全局的
`src/lvjiang/apps/yysls/core/equip_validator.py` 负责，`EquipmentParser.parse()`
解析完成后自动调用，模拟装备创建/编辑时走同一份实现。

```json
"_extra": {
  "affix_count": 5,
  "illegal_equip": ["词条 2-5 不能重复：「劲」出现了 2 次"]
}
```

| 判定码 | 含义 | 依据 |
|--------|------|------|
| `duplicate_affix` | 词条 2-5 出现重复 | 铁律一：调律词条不与已调出的重复（允许与首词条相同） |
| `attack_overflow` | 属攻类词条超过 2 条 | 铁律二：属性攻击绝不出现第三次 |
| `divine_overflow` | 神力词条超过 1 条 | 神力词条每件装备最多 1 条 |
| `transferred_divine` | 神力词条标记为转律产出 | 转律不产出神力词条 |
| `unknown_equip_type` | 装备类型未在装备配置登记 | 装备类型配置 |
| `unknown_affix` | 词条未在普通词条池登记 | 词组配置 |
| `invalid_first_affix` | 首词条不属于该部位首词条池 | `base_attrs.*._first_affixes` |
| `invalid_affix_part` | 普通词条不能出现在该装备部位 | `affix_parts` / 词组 `_parts` |
| `weapon_affix_mismatch` | 专属武学词条与武器类型不匹配 | `weapon_types[].wuxue_affix` |
| `malformed_affix` | 词条缺少有效名称或数值 | 装备数据结构 |
| `cap_overflow` | 普通词条 `cap_pct > 100` | 数值高于该等级上限，只可能是识别误读 |

规则出处见 [装备系统](../../10-game/01-equipment-system.md)。

**处置方式**：

| 场景 | 行为 | 理由 |
|------|------|------|
| 扫描（`EquipmentParser.parse`） | **只标记，绝不阻断** | 长流程，丢一件或抛一次异常就会崩坏整轮 |
| 调律过程 | **完全不判定** | 读单条新词条走 `parse_affix_text`，不经过判定器 |
| 模拟装备创建/编辑 | 组合类违规拦下不保存 | 用户在交互中，能当场改；`cap_overflow` 仍只标记 |

**「绝不阻断」是硬约束**，由三层保证，回归测试见
`tests/yysls/test_illegal_never_blocks_flow.py`：

1. `parse()` 里判定整段包在 `try` 中，判定器出任何意外都只记 error 日志，
   解析结果照常返回。上层 DSL 内置 `to_equipment()` 捕获异常后会返回空 dict，
   那等于把整件装备静默丢掉，比不标注严重得多。
2. 判定器内部对数值做防御（`cap_pct` 可能是字符串或 None），
   宁可漏报也不在扫描途中抛异常。
3. `_extra` 不参与 `_fp` 指纹计算，标注不会改变指纹，去重与存储不受影响。

库存每次加载时都会用当前游戏配置重新判定，因此历史装备也能发现新增异常；
重新判定为合法时该键会被**移除**，不会留下过期标记。

判定只作用于数据本身，与装备好坏无关——调律规则引擎不读这个字段，
它的职责是分级不是校验。全仓只有装备卡片 UI 读取该字段。

### 定音双槽与止戈定音

一件装备可以同时定着普通定音和止戈定音，游戏里随时无成本切换，所以两者
各占一个数据槽，互不覆盖：

| 字段 | 含义 |
|------|------|
| `dingyin` | 普通定音槽 `{name, value, cap_pct}` |
| `dingyin_zhige` | 止戈定音槽。真实止戈词条尚未进词库，名称固定为 `止戈定音`，结构与 `dingyin` 对齐，将来能解析出真实名称数值时直接往里填 |
| `dingyin_type` | 本次扫描时装备展示的是 `normal` 还是 `zhige`；缺失按 `normal` 读 |

一次扫描只可能读到一种定音，解析器只写对应的那一槽；另一槽由仓储写入入口
（`core/loadout/equipment_write.py`）从旧记录合并回来。词条不满 5 条时不可能
有定音，读到什么都是脏数据，整段跳过，连 `dingyin_type` 都不写。

定音不参与装备异常判定，不触发红色异常徽标。`_extra.is_zhige_dingyin` 保留
为 `dingyin_type` 的**派生**标记，供尚未改造的读取方继续工作——不得反过来
从 `dingyin.name` 推断它，普通定音槽会长期保留数据，那样每次加载都会把止戈态
改回普通态。

名称认得出但数值没读出来时按 `value: 0` 记进普通定音槽：0 在卡片上一眼就不对，
重扫一次即可恢复。

核对说明（数值未识别、疑似误读）存在**槽内**的 `notice` 字段，不放 `_extra`：
两种定音各自可能带着自己的说明，而合并时本次没带的那一槽是整块搬过去的，
说明跟着槽走才不会丢、也不会串到另一种定音头上。卡片和属性页都只读当前展示
那一槽的说明。历史记录里的全局 `_extra.dingyin_notice` 在加载时迁进它当时展示
的那一槽。

### 定音的展示与切换

「当前展示哪种定音」有两份状态，各有各的归属，互不写对方：

| 位置 | 字段 | 谁更新 |
|------|------|--------|
| 装备自身 | `dingyin_type` | 背包扫描、背包卡片上的切换 |
| 备战方案 | `LoadoutPlan.dingyin[slot]` | 备战扫描、装备栏上的切换 |

备战扫描**不更新** `dingyin_type`：切换备战方案时游戏会自动切定音，用户根本
没碰这件装备，那不是它的展示偏好。装备栏卡片按方案选择展示，背包卡片按
`dingyin_type` 展示；两槽都有数据时定音名后面挂 `⇄` 标记，属性对话框里的
「切换定音」按钮同一个判定（`can_switch_dingyin`）。按钮禁用时区分两种原因：
装备只扫描到一种定音，和当前入口不提供切换——合成一句会对着两种定音都有的
装备说假话。

槽位清空（卸下、删除装备）必须同时丢掉该槽的定音选择，统一走
`LoadoutPlan.clear_slot()`；只清装备会在磁盘上留下「空槽位仍有定音选择」的
自相矛盾状态。

毕业率与战斗属性计算完全无视这两份展示状态，只按毕业率模型读对应字段——
当前只有 PVE 模型，因此只读 `dingyin`，止戈贡献为 0。「已定音」筛选与候选池
同此口径；承音合并资格则问「定过音没有」，两种都算。

历史数据不做迁移：缺 `dingyin_type` 按 `normal` 读，旧的 `is_zhige_dingyin`
标记在加载时补出固定名称的 `dingyin_zhige` 展示槽。历史上被写进普通定音槽的
止戈名称不做识别搬迁——名称判定会随定音词库增删而翻转，重扫一次即归位。

---

## 三、词条枚举

### 3.1 外功攻击类

| 词条名 | 出现部位 |
|--------|----------|
| 最大外功攻击 | 全部位 |
| 最小外功攻击 | 武器、首饰 |

### 3.2 属性攻击类

| 词条名 | 出现部位 |
|--------|----------|
| 最大无相攻击 | 武器 |
| 最小无相攻击 | 武器 |
| 最大牵丝攻击 | 首饰、防具 |
| 最小牵丝攻击 | 首饰、防具 |
| 最大鸣金攻击 | 首饰、防具 |
| 最小鸣金攻击 | 首饰、防具 |
| 最大裂石攻击 | 首饰、防具 |
| 最小裂石攻击 | 首饰、防具 |
| 最大破竹攻击 | 首饰、防具 |
| 最小破竹攻击 | 首饰、防具 |

### 3.3 三率类

| 词条名 | 出现部位 | 单位 |
|--------|----------|------|
| 会心率 | 头/胸/胫甲/腕甲 | % |
| 会意率 | 头/胸/胫甲/腕甲 | % |
| 精准率 | 头/胸/胫甲/腕甲 | % |

### 3.4 基础属性类

| 词条名 | 出现部位 |
|--------|----------|
| 劲 | 胫甲/腕甲 |
| 势 | 武器 |
| 敏 | 武器 |
| 体 | 胫甲/腕甲 |
| 御 | 胫甲/腕甲 |

### 3.5 神力词条

| 词条名 | 出现部位 | 单位 |
|--------|----------|------|
| X武学增伤（剑/枪/扇/...） | 对应武器类型 | % |
| 全武学增效 | 环、佩 | % |
| 单体奇术增伤 | 冠胄、胸甲 | % |
| 群体奇术增伤 | 冠胄、胸甲 | % |
| 对首领单位增伤 | 胫甲、腕甲 | % |
| 对玩家单位增效 | 胫甲、腕甲 | % |

### 3.6 其他

| 词条名 | 出现部位 |
|--------|----------|
| 气血最大值 | 防具 |

---

## 四、OCR 原始输出格式

OCR 识别输出为原始文本，需经过清洗才能转换为上述结构化格式：

```json
{
  "main_weapon": {
    "equip_type": "踏雪含光 | 武器·剑",
    "equip_level": "承音 | 110阶",
    "base_attr": "外功攻击100~232",
    "affix_gong": "最大外功攻击荐114.1",
    "affix_shang": "会意率荐 6.6%",
    "affix_jue": "［转]势荐72.2",
    "affix_zhi": "最大外功攻击荐114.1",
    "affix_yu": "剑武学增伤荐9.2%"
  },
  ...
}
```

### 清洗规则

| 原始字符 | 处理 |
|----------|------|
| `荐` | 移除（OCR 噪声） |
| `[转]`、`【转]`、`［转]` | 标准化为 `is_transferred: true` |
| `\|` 分隔 | 拆分 `equip_type` 为名称和类型 |
| `承音 \|` | 解析 `is_chengyin: true` |
| `XX阶` | 提取数字为 `level` |
| `攻击100~232` | 解析为二元组 `[100, 232]` |

**数据清洗模块**：词条清洗逻辑待实现，将作为 OCR 后处理步骤集成到工作流引擎（`WorkflowEngine` + `.wf` DSL）中。

---

## 五、统计汇总格式

对 8 件装备共 40 条词条进行汇总：

```json
{
  "user": "用户名",
  "total_affixes": 40,
  "summary": {
    "最大外功攻击": 12,
    "会意率": 7,
    ...
  },
  "by_category": {
    "外功攻击": 12,
    "属性攻击": 4,
    "三率": 7,
    "基础属性": 12,
    "神力词条": 5
  }
}
```

---

## 六、装备指纹 `_fp`

每件装备在存储时携带一个 `_fp` 字段作为去重标识，用于背包管理、装备/卸载、模拟装备等场景。

**`_fp` 是 dict 内部的权威字段**，外部 dict key 只是索引别名。存储结构：

```json
"bag_items": { "weapon": { "015850f8": { ..., "_fp": "015850f8" } } }
```

其中 `"015850f8"` 是索引 key，`"_fp": "015850f8"` 是 dict 内部的权威值。

### 6.1 计算规则

```
拼接字符串 = type + level + quality + is_chengyin + affix_1~5(name:value)
_fp = MD5(拼接字符串)[:8]   # 取前 8 位十六进制字符
```

各部分用 `+` 连接，示例：

```
剑+110+gold+True+最大外功攻击:114.1+会意率:6.6+势:72.2+最大外功攻击:114.1+剑武学增伤:9.2
```

### 6.2 参与字段

| 字段 | 参与 | 说明 |
|------|------|------|
| `type` | ✓ | 装备类型（如 "剑""环"） |
| `level` | ✓ | 等级（如 110） |
| `original_level` | ✗ | 可由名称等阶重新识别，不参与实体指纹 |
| `cooldown_kind` | ✗ | 可变的冷却业务类型，不得污染实体指纹 |
| `cooldown_state` | ✗ | 可变的冷却阶段，不得污染实体指纹 |
| `cooldown_expires_at` | ✗ | 扫描时变化的冷却状态，绝对不得污染实体指纹 |
| `quality` | ✓ | 品阶（"gold""purple"） |
| `is_chengyin` | ✓ | 是否承音（True/False） |
| `lock_status` | ✗ | 可变的锁定状态，不得改变装备实体指纹 |
| `affix_1` ~ `affix_5` | ✓ | 每条词条的 `name:value` |
| `dingyin` | ✗ | **定音词条故意不包含** |
| `dingyin_zhige` | ✗ | 同上，止戈定音槽一样是可变状态 |
| `dingyin_type` | ✗ | 纯展示状态，更不得参与 |
| `base_attr` | ✗ | 基础属性不参与 |
| `_extra` | ✗ | 扩展标记不参与 |
| `_fp` | ✗ | 指纹自身不参与计算 |

**定音词条不参与指纹的原因**：装备可能切换定音，五个词条数值完全相同的概率极低，包含定音会导致同一件装备换定音后被误判为不同装备。指纹是实体身份，定音是可变状态——两种定音并存正是靠这条划分才能存在同一条记录上。

### 6.3 自动产出与模拟装备

`EquipmentData.to_dict()` 默认自动计算并包含 `_fp`：

```python
# 真实装备
equip_dict = equipment_data.to_dict()          # _fp = "015850f8"

# 模拟装备
equip_dict = equipment_data.to_dict(is_mock=True)  # _fp = "mock_015850f8"
```

`make_fingerprint()` 同样支持 `is_mock` 参数：

```python
make_fingerprint(equip_dict)                    # "015850f8"
make_fingerprint(equip_dict, is_mock=True)      # "mock_015850f8"
```

这确保模拟装备指纹与真实装备指纹永不冲突。**每次编辑模拟装备的词条/数值后必须重新生成指纹。**

### 6.4 扫描装备的受限养成

扫描装备不开放通用编辑，只允许保存游戏中发生的单向变化：承音提升到下一配置等级、
现有普通词条和定音数值只增不减，以及商角徵羽中单个固定槽位的转律。定音名称、装备
身份、品阶、原始等级和基础属性不可修改，空词条不可补造。定音数值不参与指纹，因此
单独培养定音会原位更新；其他参与指纹的变化通过 `update_real_development()` 原子写入
新指纹、迁移全部方案引用并删除旧指纹。

转律是唯一会重置冷却的养成行为。冷却天数来自 `game_config/basic.yaml` 的
`basic_config.equipment_cooldown_days`，缺省为 5。

### 6.5 库存时间元数据

装备进入备战方案的 fp 共享池后，单件数据额外包含：

| 字段 | 语义 |
|------|------|
| `created_at` | 该 fp 第一次写入共享池的 UTC ISO 时间 |
| `updated_at` | 该 fp 最近一次被重新写入的 UTC ISO 时间 |

同一 fp 再次扫描、编辑或覆盖时保留 `created_at` 并刷新 `updated_at`。历史数据
没有时间字段时按空字符串处理，不推断为当前时间，更不能显示成 1970 年。
承音装备快照合并后，保留版本取整组最早的非空 `created_at` 和最新的非空
`updated_at`；整组都没有对应时间时结果仍为空。这两个字段不参与 `_fp` 计算。

### 6.6 存储位置

| 类别 | JSON 节点 | 说明 |
|------|-----------|------|
| 真实装备 | `bag_items` | 按部位分组，以 `_fp` 为索引 key |
| 模拟装备 | `mock_items` | 结构同上，索引 key 带 `mock_` 前缀 |

两者结构相同（`{ group_key: { _fp: equip_data } }`），但在扫描工具 (wf) 扫描时必须分离，因此分开存储。UI 展示时将两者合并，模拟装备卡片带「模拟」标签。

...
默认模式只做参考图匹配，不执行图库 `meta_schema` 中输出区域的 OCR；
只有显式写出 `as rich` 才会读取 `level_text`、`count_text` 等输出字段。
只需要材料名、置信度过滤或命中位置时，应使用默认模式或 `by`。
...
```text
recognize [scene].[card_1].[icon] as $item
recognize [scene].$card.[icon] as $item
```
...
| `rich` | `as rich $var` | 启用输出区域 OCR，返回完整识别信息 |
...
这也是 `recognize` 唯一会执行输出区域 OCR 的模式。
...
> 游戏插件通过 `@builtin_func("func_name")` 提供转换函数，DSL 中用 `with func_name` 显式指定。识别本身由通用的 `core/recognizers/ReferenceRecognizer` 完成，不掺任何插件领域知识；富 dict 的字段解析全部经 `with` 指定的插件函数完成。

### rich 与 by 互斥

`rich` 和 `by` 会产生不同的返回类型，不能同时使用：

- **`rich` 是升级**：str → 富 dict
- **`by` 是降级**：dict → str/位置
- **组合即报错**：解析器会说明两者的返回语义冲突，不进入执行阶段
...
- **输出 OCR 按需执行**：plain 与 `by` 只做参考图匹配；只有
  `as rich` 才读取图库输出字段
- **`as rich` 与 `by` 互斥**：组合使用会在解析阶段抛出 `WorkflowUserError`
- **识别器生命周期**：`ReferenceRecognizer` 随 `WorkflowEngine` 实例创建（`core.py` 构造函数里 `self._reference_recognizer = ReferenceRecognizer(self._ocr)`），不是类级别共享单例；参考图库的加载/刷新由 `core.reference_db.ReferenceDatabase` 独立管理

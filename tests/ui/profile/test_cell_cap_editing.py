...
1. 剥离 /cap 的前置判断只认 regen / quota，stock 和 note 同样会显示 X/Y
   却被漏掉；
2. 取 KeyDef 时用了 column_keys[col]，但第 0 列是用户名、数据列从 1 开始，
...
import lvjiang.ui.profile.cell_editing as cell_editing
...
def _build_table(env, qtbot) -> QTableWidget:
    """按总览的真实列布局建表：第 0 列用户名，之后依次是三列数据。"""
    from lvjiang.core.profile.repository import db_read_all

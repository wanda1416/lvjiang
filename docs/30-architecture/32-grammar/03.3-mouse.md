...
仅移动鼠标到目标位置，不点击。与 `click` 共享完全相同的目标语法。

**语义**：`move` 将目标解析为屏幕坐标后移动鼠标，不发送点击事件。

**语法**：

```
move [name].[name]            # 配置引用（场景.区域）
move [scene].[panel][r][c]    # Panel 三级索引（移动到格子中心）
move [scene].[panel]          # 移动到 Panel 中心
move $var                     # 移动 CoordRef / find 产出的文字区域
move (rx, ry)                 # 画布归一化坐标
```
...
```
# 悬停查看装备属性
move [inventory].[slot3]
wait 0.5
scan [tooltip].[text] as $desc

# 确认后再点击
if $desc contains "金色"
    click [inventory].[slot3]
end

# 安全区检查：先移动确认位置，再决定点击
move [scene].[target]
wait 0.2
scan [scene].[verify] as $check by contains "确认"
if $check
    click [scene].[target]
end
```
...
| 后端 | 实际行为 | 效果 |
|------|----------|------|
| **SendInput** | `SetCursorPos(x, y)` | ✅ 移动系统光标，所有程序可见 |
| **PostMessage** | `PostMessage(hwnd, WM_MOUSEMOVE, ...)` | ⚠️ 只更新目标窗口消息队列，不移动系统光标 |
| **ADB** | 空操作 | ❌ 设备端无鼠标概念 |

**与 click 的区别**：

- `move` **没有**默认 before/after 延迟（click 有隐藏延迟）
- `move` 支持 `before`/`after`/`around` wait_clause，但不抑制默认延迟（因为没有默认延迟）
- 目标解析逻辑与 `click` 完全一致：EntityRef → PanelRef → VarRef → CoordPoint

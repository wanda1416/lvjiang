...
@pytest.mark.parametrize("kind", ["region", "point", "panel"])
@pytest.mark.parametrize("hot_reload", [False, True])
@pytest.mark.parametrize("collision", [False, True])
def test_entity_rename_preserves_all_references_or_rejects_before_write(
        tmp_path, kind, hot_reload, collision):
    scenes = tmp_path / "scenes"
    scenes.mkdir()
    documents = {
        "source": {"key": "source", "name": "源",
                   kind + "s": [{"key": "old", "name": "旧", "type": "func"}]},
        "first": {"key": "first", "name": "引用方一",
                  "references": [{"scene": "source", "entity": "old"}]},
        "second": {"key": "second", "name": "引用方二",
                   "references": [{"scene": "source", "entity": "old"}]},
    }
    if collision:
        documents["second"]["regions"] = [{"key": "new", "name": "占用", "type": "func"}]
    for key, doc in documents.items():
        (scenes / f"{key}.yaml").write_text(yaml.safe_dump(doc), encoding="utf-8")
    resolver = ConfigResolver(system_dir=tmp_path, local_dir=tmp_path, dev_mode=True)
    registry = SceneRegistry(resolver=resolver)
    if hot_reload:
        def reload_in_place(_path):
            refreshed = SceneRegistry(resolver=resolver)
            registry.__dict__.clear()
            registry.__dict__.update(refreshed.__dict__)
        resolver.add_change_listener(reload_in_place)
    before = {path: path.read_bytes() for path in scenes.iterdir()}
    rename = getattr(registry, f"rename_{kind}_key")
    if collision:
        with pytest.raises(ValueError):
            rename("source", "old", "new")
        assert {path: path.read_bytes() for path in scenes.iterdir()} == before
        assert getattr(registry.get_scene("source"), kind + "s")[0].key == "old"
    else:
        rename("source", "old", "new")
        for current in (registry, SceneRegistry(resolver=resolver)):
            for owner in ("first", "second"):
                refs = current.get_scene(owner).references
                assert [(ref.scene, ref.entity) for ref in refs] == [("source", "new")]

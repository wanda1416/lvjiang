...
class RunControlMixin:
    ...
    def run_batch(self, users, scripts, skip_first_switch: bool = True) -> bool:
        """批量执行入口（批量 Tab 调用）：多用户 × 多脚本顺序执行。

        Args:
            users: list[BatchUser] 勾选的用户（按执行顺序）
            scripts: list[BatchScript] 勾选的脚本
            skip_first_switch: 首个用户是否跳过账号切换（已登录）
        ...
        worker = BatchWorker(
            ...
            stop_check=self._is_stopped,
            skip_first_switch=skip_first_switch,
        )

        # 信号接线：进度/日志 → UI，结束 → 恢复状态
        tab = self._batch_tab
        if tab is not None:
            worker.progress.connect(tab._update_progress)
            worker.finished_all.connect(tab._on_batch_finished)
        worker.log.connect(self.log_text.append)
        worker.finished_all.connect(lambda _: self._end_automation("批量执行"))

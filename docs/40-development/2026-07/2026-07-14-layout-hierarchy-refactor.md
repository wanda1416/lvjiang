...
### 13. OCR 识别方案：回归逐区域裁剪识别

**现象**：覆盖裁剪 + 一次推理方案虽然快，但导致各区域错误检测率上升（整图推理时背景干扰增多）。

**方案**：改回逐区域裁剪后单独识别，牺牲少量速度换取识别准确率。
...
**变更范围**：
- 新增 `lvjiang/core/user_config.py` — 用户数据管理（User 数据类 + UserManager）
- 新增 `lvjiang/ui/user_manager_dialog.py` — 用户管理对话框
- 修改 `lvjiang/ui/main_window.py` — 顶部用户选择器 + 菜单启用用户管理

**功能实现**：
- 主页面顶部新增用户选择器下拉框，可切换当前用户
...
```json
{
  "users": [{"name": "默认用户", "created_at": "2026-07-14T..."}],
  "active_user": "默认用户"
}
```
...
**变更范围**：
- 修改 `lvjiang/ui/main_window.py` — `_setup_menu` 重写 + `_show_about` 新增

**菜单结构**：
- 设置：用户管理（已启用）、区域编辑（已启用）
- 工具：毕业率计算器（禁用）、图像识别测试（已启用）
- 帮助：关于（简单说明对话框）

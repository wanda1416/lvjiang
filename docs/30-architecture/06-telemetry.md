...
改成"一次调律会话存一行、`payload` 整个存 JSON 数组"，写入量降到约
`DAU × 每日会话数`，与单次会话里调律多少次无关，可以撑到约 4~5 万 DAU。
...
`part`/`food`/`mode`/`quality`/`run_env`/`os_name`/`plugin` 是服务端能
穷举的封闭枚举，不在名单内直接丢弃该条，这条是结构性保证。

`affix`/`weapon_type` 做不到同等强度——它们的取值随游戏配置增长，服务端
没有 `game_config.yaml` 作为真源。退而求其次的格式闸门是"1~16 个纯中文
字符"，挡得住路径/邮箱/账号这类几乎总带 ASCII 的 PII 形状，**挡不住一个
纯中文的伪造字符串**（例如故意填"角色名叫张三"）——这一点已用真实请求

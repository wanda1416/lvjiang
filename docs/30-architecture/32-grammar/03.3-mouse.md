...
scroll up|down [scene].[region]      # 移动到目标位置后滚动 1 格
scroll up|down [scene].[region] N    # 移动到目标位置后滚动 N 格
scroll up|down $var                  # 移动到变量指向的位置滚动
scroll up|down [scene].[panel][r][c] # 移动到格子中心后滚动
scroll up|down (rx, ry)             # 移动到坐标位置后滚动
scroll up|down [target] $n           # 滚动数量由变量控制
```
...
scroll down [inventory].[list]
scroll up [inventory].[list] 5

# 动态数量
scroll down [bag_equip_detail].[bag_grid] $scroll_amount
...
| 后端 | 实际行为 | 说明 |
|------|----------|------|
| **SendInput** | `SetCursorPos` + `MOUSEEVENTF_WHEEL` | ✅ 移动系统光标 + 发送滚轮事件 |
| **PostMessage** | 坐标转换 + `WM_MOUSEWHEEL` | ⚠️ 向目标窗口投递滚轮消息 |
| **ADB** | `input swipe` 短距离滑动 | ⚠️ 用触摸滑动模拟滚轮，每格 100px |
| **设备端** | 空操作 | ❌ 不支持 |

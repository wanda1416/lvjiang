...
# ── 带 with 子句（OCR 清洗分组） ──
find as $var by contains "文字" with "清洗组名"
find [scene].[area] as $var by contains "文字" where confidence >= 0.8 with "清洗组名"
```
...
所有 OCR 结果都会先经过全局默认清洗规则。`with "<清洗组>"` 会在默认规则
之后叠加配置中已定义的业务规则组，写在 `by` / `where` 之后：

```text
find as $found by contains "调律" with "装备词条"
find [scene].[area] as $btn by contains_any $list where confidence >= 0.7 with "装备词条"
```

- 参数只能是字符串常量（清洗组名），不接受变量、模板名或函数名；
- 组名必须存在于清洗配置，否则执行期直接报错；
...
`by image` 的 target 可为当前布局的 Region 模板绑定，也可为显式模板名或变量。
完整匹配语义见 [04.4-image-template.md](04.4-image-template.md)。

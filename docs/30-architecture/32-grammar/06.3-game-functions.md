...
根据词条名和装备等级查询数值上限。`affix_cap` 返回上限值，`chengyin_cap` 返回承音上限（词组配置里的原值，不按比例推算）。自动映射词条名（如 "最大外功攻击" → "外功攻击"），找不到返回 0。
...
### scanned_loadout_names / write_bag_item / write_equipped / set_scanned_loadout_gongjue — 装备与方案写入

| 函数 | 签名 | 说明 |
|------|------|------|
| `scanned_loadout_names` | `() -> dict` | 读取本地已有方案名称快照，供批量任务在切换游戏方案前判断是否跳过 |
| `write_bag_item` | `(group_key, dict) -> str` | 把背包装备写入用户装备库，返回写库指纹；参数必须是字典，且 `type` 与 `group_key` 不一致时抛 `ValueError` |
| `write_equipped` | `(slot_key, dict) -> str` | 把装备写入本次扫描绑定方案的指定槽位，返回指纹；方案 ID 缓存在 `context._bound_loadout_plan_id` |
| `set_scanned_loadout_gongjue` | `(gongjue) -> str` | 把 `会意`、`会心` 或 `精准` 写入本次扫描绑定方案的弓玦套装；未绑定方案或值不合法时报错，不改变主页面活动方案 |

三者写库后都会广播「装备变更」事件，UI 侧无需轮询。
...
```text
# 已分别从 detail_1、展开的属性攻击 detail_2 读取 $left 和 $right
eval $attrs = to_role_base_attrs({"left_1": $left.detail_1, "right_attr_attack": $right.detail_2})

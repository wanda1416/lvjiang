...
字段来源于 Excel 计算器的输入参数和游戏内战斗属性面板。
...
#
# 劲: 1点劲 = 0.225小外攻 + 1.36大外攻
# 势: 1点势 = 0.9大外攻   + 0.038%会意率
# 敏: 1点敏 = 0.9小外攻   + 0.076%会心率
# 体: 1点体 = 60生命值（当前系统不追踪）
# 御: 1点御 = 17生命值 + 0.5防御（当前系统不追踪）
#
# 系数与等级无关，故不随 affix_caps 分档：国服 110 级的实测值与
# where-builds-meet（国际服 96 级）的 system.json baseAttributes 在
# 敏、势的四个系数上逐位相同，劲的大外攻也相同。两个不同服、不同
# 等级得出同一组数，等级无关成立。
#
# 校验不变量：一条满值五维词条产出的各项，按各自词条满值归一后
# 相加应为 1（见 tests/yysls/test_combat_attrs.py 的归一测试）。
# 110 级满值下：劲 1.003、势 0.986、敏 0.986。
#
# 历史：早期这里是一组自行拟合的值（劲 0.246/1.315、势 0.9/0.04%、
# 敏 1.0/0.075%），敏归一后达 1.044——超过一整条词条，不可能成立。
# 该错误经反推路径（面板 - 装备 = 基础属性，见 play_style_dialog）
# 传进基础属性，使五维词条的边际收益被系统性高估约 11%。
JIN_TO_MIN_OUTER = 0.225     # 1劲 → 最小外功攻击
JIN_TO_MAX_OUTER = 1.36      # 1劲 → 最大外功攻击
SHI_TO_MAX_OUTER = 0.9       # 1势 → 最大外功攻击
SHI_TO_INTENT_RATE = 0.00038  # 1势 → 0.038%会意率（小数形式）
MIN_TO_MIN_OUTER = 0.9       # 1敏 → 最小外功攻击
MIN_TO_CRIT_RATE = 0.00076   # 1敏 → 0.076%会心率（小数形式）
...
def convert_five_dims(jin: float = 0, shi: float = 0, min_val: float = 0,
    ...
    转换公式（系数与出处见模块顶部的五维转换系数段）：
    - 劲: 1点 → 0.225小外攻 + 1.36大外攻
    - 势: 1点 → 0.9大外攻 + 0.038%会意率
    - 敏: 1点 → 0.9小外攻 + 0.076%会心率

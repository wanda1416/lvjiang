...
def test_large_yellow_distractor_does_not_outweigh_center_arrow():
    img = _concave_arrow_image(75, size=240)
    cv2.rectangle(img, (150, 80), (220, 150), YELLOW_BGR, -1)

    result = detect_arrow_heading(img, center=(120, 120), window_ratio=1.0)

    assert result is not None and _angle_diff(result.heading_deg, 75) <= 3.0

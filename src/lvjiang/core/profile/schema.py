...
def _load_config() -> ProfileSchema:
    ...
    _validate_periods(keys_by_model)
    return ProfileSchema(keys_by_model=keys_by_model)


def _validate_periods(keys_by_model: dict[str, list[KeyDef]]) -> None:
    """在配置加载阶段拒绝未注册周期，避免后台 tick 持续失败。"""
    for key_def in keys_by_model.get("quota", []):
...
def save_profile_config(schema: ProfileSchema) -> None:
    """保存配置到 profile.yaml"""
    try:
        _validate_periods(schema.keys_by_model)
        data = schema.to_dict()

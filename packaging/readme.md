...
仓库级授权边界、普通提交/推送规则以及发布前提交整理原则见
[`AGENTS.md`](../AGENTS.md)。若需要改写 `master..dev` 历史，必须先按其中的备份、
tree hash 校验和精确 `--force-with-lease` 流程执行；本文件继续作为版本准备、标签、
打包和 GitHub Release 的完整规范。

“准备发布”与“正式发布”是两个阶段：前者完成版本文件、发布说明和验证，不自动授权
合入 `master` 或推送标签；后者才执行下文第四、第五步。若开发者要求二次确认，AI Agent
必须在更新 `master` 和创建标签前停下等待。

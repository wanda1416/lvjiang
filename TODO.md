...
| **脚本工作台** | 完成 + 打磨中 | ✅ | 🟡 中 |
| **图色原语** | 完成 + 真机验证待 | ✅ | 🔴 高 |
| **窗口模式** | Phase 0（诊断） | ⚠ 已知回归 | 🔴 高 |

---

## 🔴 P0 关键阻塞（当前）

### 1. 窗口模式后台投递严重回归 {#p0-post-message}

**位置**: `src/lvjiang/core/desktop/post_message.py:4`
...
**上下文**:
- 概率性失效：后台点击（启用「后台模式」时）偶发不落地
- 影响面：桌面投屏模式的完整性（当前建议通过 ADB/scrcpy 绕开）
- 未定根因：可能是 Win32 API 序列、窗口焦点、消息队列竞态

**待办**:
- [ ] 原因诊断：对比历史版本定位回归节点
- [ ] 搭建复现环境（Windows 投屏 + 游戏后台）
- [ ] 验证修复方案
- [ ] 文档更新：明确后台模式的平台支持状态

...
def test_migrating_an_entity_retargets_the_references(referenced) -> None:
    ...
    referenced.create_scene("destination", "目标")
    referenced.add_region_to_scene("destination", RegionDef("ok_btn", "确认", type="func"))
    changed = referenced.retarget_references(
        "general_control", "destination", "confirm", "ok_btn")

    assert sorted(changed) == ["bag", "equip_tune_detail"]
    ref = referenced.get_scene("equip_tune_detail").references[0]
    assert (ref.scene, ref.entity) == ("destination", "ok_btn")
    # 改指之后源场景就删得掉了
    referenced.remove_region_from_scene("general_control", "confirm")
...
def test_migration_preflight_rejects_self_reference_and_name_collision(referenced):
    from lvjiang.core.scene_definition_models import RegionDef

    with pytest.raises(ValueError, match="不能引用自身"):
        referenced.validate_reference_retarget("general_control", "bag", "confirm", "ok_btn")
    referenced.add_region_to_scene("bag", RegionDef("occupied", "已有", type="func"))
    with pytest.raises(ValueError):
        referenced.validate_reference_retarget("general_control", "destination", "confirm", "occupied")
    for key in ("bag", "equip_tune_detail"):
        assert referenced.get_scene(key).references[0].entity == "confirm"

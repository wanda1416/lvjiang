...
# 材料 label：须与图库空间「调律材料」组的参考图 label 严格一致，
# 材料识别按 label 精确匹配。
#
# 这几个是**数据键**，一律不过 tr()：它们既要匹配参考图库的 label，也要
# 校验调律规则 YAML 里写的狗粮名、还要做遥测词表映射，三处都是中文数据，
# 与界面语言无关。过 tr() 只要有人补上英文翻译，英文界面下材料匹配就会
# 整条失效——而这类失效是静默的（匹配不上=当作没有该材料）。
# label 的归属长远应该在参考图库自身，而不是 i18n 译表。
STONE_LABEL = "大律准石"
FOOD_LABELS = ("金狗粮", "紫狗粮", "彩狗粮")
# 小律准石：一键添加会把它一次性用光，图标随之从材料区消失、后面的槽位

...
from lvjiang.ui.profile.cell_editing import _PARSE_ERROR, _parse_value


def test_parse_value_allows_float_only_when_decimal(monkeypatch):
    monkeypatch.setattr(
        "lvjiang.ui.profile.cell_editing.QMessageBox.warning",
...
def decimal_env(tmp_path):
    ...
    profile_config._PROFILE_PATH.write_text(
        yaml.dump(
            {
                "stock": [
                    {"key": "decimal_resource", "label": "小数资源", "decimal": True},
                    {"key": "tokens", "label": "代币"},
                ],
            },
            allow_unicode=True,
...
def test_profile_action_normalizes_tiny_float_noise(decimal_env):
    ...
    result = profile_action(decimal_env.username, "decimal_resource", set_value=12.0000000001)
    entry = db_read_entry(decimal_env.username, MODEL_STOCK, "decimal_resource")
...
def test_profile_action_keeps_real_decimal(decimal_env):
    ...
    result = profile_action(decimal_env.username, "decimal_resource", set_value=12.5)
    entry = db_read_entry(decimal_env.username, MODEL_STOCK, "decimal_resource")
...
def test_profile_action_does_not_block_real_decimal_on_non_decimal_key(decimal_env):
    ...
    result = profile_action(decimal_env.username, "tokens", set_value=12.5)
    entry = db_read_entry(decimal_env.username, MODEL_STOCK, "tokens")

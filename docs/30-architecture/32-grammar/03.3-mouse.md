...
**平台支持**：仅桌面 SendInput 后端完整支持全部五种键；PostMessage 后端目前暂不可用（见模块说明），非左键会降级为左键并记警告；ADB / 设备端触屏后端没有鼠标键概念，非 `left` 会被忽略（仍执行普通点击）并记警告。
...
- **低精度**：连续 Raw Input 包按空闲间隔划分移动段，每段合并为一条
  `move by`；点击、拖拽、滚轮和按键仍生成普通、可编辑的 WF 指令。

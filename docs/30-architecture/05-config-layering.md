...
一次发布代次，不是编辑次数；尤其调律规则是自动保存，若每次控件变化都提升，
很快就失去维护意义。
...
- 调律规则在规则 key 右侧显示版本与「提升」按钮；按钮是明确的即时操作，
  点击后立即保存并 reload 生效。
...
- **日志**：`ConfigResolver._log_supersede` 在远程顶替系统时记一条
  ...
  还会逐个调，不去重会刷屏到没人看日志。
- **界面**：场景编辑器的「视图」行分别显示
  `场景版本号：v4 [提升至 v5]`与`布局版本号：v1 [提升至 v2]`。
  版本值以颜色区分来源：系统灰色、用户蓝色、远程橙色，悬停时显示
  完整来源说明。没人会盯着日志编辑配置，界面上随时可见才真正
  解决问题。数据来自 `ConfigResolver.describe_entity()`，由
  `ui/scene_editor/scene_tab.py` 组装展示。

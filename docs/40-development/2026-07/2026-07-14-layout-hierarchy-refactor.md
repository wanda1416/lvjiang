...
**新结构**：
```
[布局栏] 当前布局 XXX | 激活 | 新建 | 加载 | 保存 | 另存为 | 删除 | 刷新截图
...
**布局栏语义**：
- 左侧显示「当前布局 XXX」，无激活布局时红色显示「无布局」
- **激活**：将当前加载的布局设为 active
- **新建**：输入名称 → 创建空布局 → 设为 active
- **加载**：弹出选择对话框 → 加载布局到所有 tab 画布
- **保存**：从所有 tab 收集 regions → 全量写入当前布局文件
...
### 4. 布局栏 UI 重构：下拉框 → 标签+按钮

**现象**：布局下拉框操作不直观，加载/删除/另存为等操作缺少确认流程。

**修复**：移除 `QComboBox`，左侧改为「当前布局 XXX」标签（无布局时红色显示「无布局」），右侧放置功能按钮：激活、新建、加载、保存、另存为、删除。加载和删除改为弹出选择对话框，另存为支持覆盖确认，激活布局不可删除。

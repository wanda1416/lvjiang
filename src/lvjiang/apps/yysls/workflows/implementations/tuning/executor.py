...
class TuningExecutor:
    ...
    def _accept_stone_scan(
        ...
        *,
        ...
        if stock is None:
            return
        units = self._stone_scan_units(deduped)
        if validate_cache and units is not None:
    ...
    def _stone_scan_units(self, deduped: dict[str, object]) -> int | None:
        """统一折算大、小石头；识别到材料但数量未知时不能沿用旧读数。"""
        units = 0
        for label, factor in ((STONE_LABEL, 10), (SMALL_STONE_LABEL, 1)):
            info = deduped.get(label)
            if info is None:
                continue
            if not self._has_recognized_count(info):
                return None
            units += int(self._get_count(info) or 0) * factor
        return units
    ...
    def _check_stone_stock(self, settings: MaterialSettings,
        ...
        if infos is not None:
            deduped = self._dedup_by_confidence(infos)
            strategy.accept_scan(self._stone_scan_units(deduped))

...
from lvjiang.core.profile.sync import (
    _apply_ratio,
    fire_sync_targets,
)
...
class TestApplyRatio:
    ...
    def test_decimal_stock_keeps_float(self):
        result = _apply_ratio(
            ...
            0.25,
            MODEL_STOCK,
            StockKeyDef(key="decimal_resource", label="小数资源", decimal=True),
        )
        assert result == 2.5
        assert isinstance(result, float)
...
class TestFireSyncTargets:
    ...
    def test_multi_target(self, monkeypatch):
        ...
        source_kd = QuotaKeyDef(
            key="monthly_task", label="月任务",
            sync_targets=[
                SyncTargetDef(key="stock:credits", ratio=1.0),
                SyncTargetDef(key="stock:reserve", ratio=-1.0),
            ],
        )
        schema = _make_schema(
            (MODEL_QUOTA, source_kd),
            (MODEL_STOCK, StockKeyDef(key="credits", label="积分")),
            (MODEL_STOCK, StockKeyDef(key="reserve", label="储备")),
        )
        monkeypatch.setattr(
            "lvjiang.core.profile.sync.get_profile_config",
            lambda: schema,
        ...
        assert write_fn.calls[0]["key"] == "credits"
        assert write_fn.calls[0]["delta"] == 100
        assert write_fn.calls[1]["key"] == "reserve"
    ...
    def test_cross_model_stock_to_quota(self, monkeypatch):
        ...
        source_kd = StockKeyDef(
            key="reserve", label="储备",
            sync_targets=[SyncTargetDef(key="quota:monthly_task", ratio=-1.0)],
        )
        schema = _make_schema(
            (MODEL_QUOTA, QuotaKeyDef(key="monthly_task")),
    ...
    def test_cross_model_regen_to_stock(self, monkeypatch):
        ...
        source_kd = RegenKeyDef(
            key="energy", label="能量",
            sync_targets=[SyncTargetDef(key="stock:credits", ratio=0.5)],
        )
        schema = _make_schema(
    ...
    def test_direction_pos_fires_only_on_increase(self, monkeypatch):
        """正向限定：仅源数值增加时触发"""
        source_kd = StockKeyDef(
            key="source_counter", label="来源计数",
            sync_targets=[SyncTargetDef(key="quota:target_progress", direction=DIR_POS)],
        )
        schema = _make_schema(
            (MODEL_STOCK, source_kd),
            (MODEL_QUOTA, QuotaKeyDef(key="target_progress")),
        )
        monkeypatch.setattr(
            "lvjiang.core.profile.sync.get_profile_config",
            lambda: schema,
    ...
    def test_direction_neg_fires_only_on_decrease(self, monkeypatch):
        """负向限定：仅源数值减少时触发（用户场景：减少来源计数才同步进度）"""
        source_kd = StockKeyDef(
            key="source_counter", label="来源计数",
            sync_targets=[SyncTargetDef(key="quota:target_progress", direction=DIR_NEG)],
        )
        schema = _make_schema(
            (MODEL_STOCK, source_kd),
            (MODEL_QUOTA, QuotaKeyDef(key="target_progress")),

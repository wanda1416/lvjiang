...
from ...core.material_db import MaterialDatabase
...
class MaterialManagerDialog(QDialog):
    ...
    def _build_add_tab(self) -> QWidget:
        ...
        self._grid_panel.cut_requested.connect(self._on_cut_requested)
        self._grid_panel.gap_changed.connect(self._canvas.set_grid_gap)
    ...
    def _on_cut_requested(self, rows: int, cols: int):
        """处理切割请求：在画布上显示网格预览"""
    ...
    def _on_submit_cells(self, cells_data: list[tuple]):
        ...
        if count > 0:
            ...
            self._grid_panel.set_known_groups(groups)

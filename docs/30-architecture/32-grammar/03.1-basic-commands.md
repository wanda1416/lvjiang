...
> 交互指令概览见 [03-interaction.md](03-interaction.md)。感知类指令（scan/recognize/find/where）见 [04-data-flow.md](04-data-flow.md) 概览及 [04.1-scan.md](04.1-scan.md) / [04.2-recognize.md](04.2-recognize.md) / [04.3-find.md](04.3-find.md) / [04.4-image-template.md](04.4-image-template.md) 详细文档。

## 目录

- [一、collect — 收集输出](#一collect--收集输出)
- [二、eval — 赋值](#二eval--赋值)
- [三、default — 默认值赋值](#三default--默认值赋值)
- [四、call — 调用子过程](#四call--调用子过程)
- [五、log — 日志输出](#五log--日志输出)
...
```
default $var = <literal>     # literal 可以是字符串、数字、泛化元组、字典、列表
```
...
default $slots = {"head": true}      # 字典（checkgroup 参数的兜底值）
```
...
工作流参数的声明方式，以及"每个参数都该配一条 `default`"的理由，见
[07-subworkflows.md](07-subworkflows.md#六工作流参数声明)。

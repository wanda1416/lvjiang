...
def test_validate_rejects_missing_input_trace(tmp_path):
    wf = tmp_path / "missing.wf"
    wf.write_text(
...
def test_validate_covers_input_traces_inside_imported_procs(tmp_path):
...
def test_imported_proc_input_trace_resolves_relative_to_its_own_file(tmp_path):
    """导入过程里的相对轨迹路径相对该过程自己的文件解析，不是根文件。"""
    lib_dir = tmp_path / "lib"
    trace_dir = lib_dir / "lvtrace"
    ...
    # 根目录下故意不放 lvtrace/，证明解析没有落到根目录的 base_dir 上。
    assert not (tmp_path / "lvtrace").exists()
...
def test_validate_rejects_input_trace_path_traversal_outside_workflow_tree(
    ...
    workflows_root = tmp_path / "workflows_root"
    (workflows_root / "workflows" / "standalone").mkdir(parents=True)
    outside_trace_dir = tmp_path / "outside" / "lvtrace"
...
def test_engine_replays_valid_input_trace_through_single_backend_call(tmp_path):
    trace_dir = tmp_path / "lvtrace"
    trace_dir.mkdir()

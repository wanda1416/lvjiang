...
class EquipmentParser:
    ...
    def _parse_weapon_base(self, raw: str) -> BaseAttr | None:
        """解析武器基础属性

        "外功攻击100~232"  → BaseAttr("外功攻击", [100, 232])
        "外功攻击 60~140"  → BaseAttr("外功攻击", [60, 140])
        "外功攻击 老著 52~121" → BaseAttr("外功攻击", [52, 121])  [脏]
        """
        raw = raw.strip()
        if not raw:
            return None
        ...
        if len(nums) >= 2:
            return BaseAttr(name="外功攻击", value=[int(nums[0]), int(nums[1])])

        logger.warning(f"武器 base_attr 无法解析: {raw!r}")
        return None

    def _parse_jewelry_base(self, raw: str) -> BaseAttr | None:
        """解析首饰基础属性

        "最小外功攻击 133" → BaseAttr("最小外功攻击", 133)
        "最大外功攻击 199" → BaseAttr("最大外功攻击", 199)
        """
        return self._parse_single_value_attr(raw)

    def _parse_armor_base1(self, raw: str) -> BaseAttr | None:
        """解析防具基础属性1

        "气血最大值8750"    → BaseAttr("气血最大值", 8750)
        "气血最大值 7778"   → BaseAttr("气血最大值", 7778)
        "外功攻击 花著 52~121" → 脏数据，尝试提取
        """
        return self._parse_single_value_attr(raw)

    def _parse_armor_base2(self, raw: str) -> BaseAttr | None:
        """解析防具基础属性2

        "外功防御34"     → BaseAttr("外功防御", 34)
        "外功防御 20"    → BaseAttr("外功防御", 20)
        "外功防御 生无在病27" → BaseAttr("外功防御", 27)  [脏]
        """
    ...
    def _parse_single_value_attr(
        ...
        for attr_name in ["气血最大值", "外功防御", "最大外功攻击", "最小外功攻击"]:
            if raw.startswith(attr_name):
                ...
                if nums:
                    return BaseAttr(name=attr_name, value=int(nums[-1]))
                return None
        ...
        if expected_name:
            nums = re.findall(r"\d+", raw)
            if nums:
                return BaseAttr(name=expected_name, value=int(nums[-1]))

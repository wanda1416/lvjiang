...
定义与存储完全镜像：profile.yaml 按模型归档，profile.db 通过 type 字段区分模型。
"""
...
class SyncTargetDef:
    ...
    key 采用 `model_type:key` 命名空间（如 `stock:credits`），跨模型无歧义。
...
def parse_sync_key(raw: str) -> tuple[str, str]:
    ...
    裸 'credits'   → ('', 'credits')（隐式模型，由调用方按上下文推断）
    """
    raw = (raw or "").strip()
    if ":" in raw:
...
def format_sync_label(sync_key: str) -> str:
    ...
    'stock:credits' → '库存：积分'
    'quota:weekly_task' → '配额：周任务'
    解析失败降级到原始 ID。
    """
    model_type, key = parse_sync_key(sync_key)
    # 延迟导入避免 models 与 schema 循环依赖。
    try:

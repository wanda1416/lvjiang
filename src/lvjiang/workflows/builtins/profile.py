...
def _profile_inc(_engine, key: str, delta=1, *args) -> float:
    ...
    .wf 用法:
        ...
        # 消耗通用资源
        eval $energy = profile_inc("energy", -30)
    """
    if not key:
...
def _profile_model(_engine, key: str, *args) -> str:
    ...
    .wf 用法:
        eval $model = profile_model("weekly_task")
...
def _profile_all(_engine, *args) -> dict:
    ...
    走共享读取管线 core.profile.service.profile_read_all()，与 UI 读取路径一致：
    ...
    """
    from ...core.profile.service import profile_read_all
    username = _get_username(_engine)
